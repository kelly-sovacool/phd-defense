---
title: "Improving machine learning models of the human gut microbiome"
...
metadat_cases <- read_csv('severe-CDI/data/process/cases_full_metadata.csv') %>% 
    filter(!(is.na(idsa) & is.na(allcause) & is.na(attrib) & is.na(pragmatic)))
perf_dat <- read_csv('severe-CDI/results/performance_results_aggregated.csv') %>% 
    rename(AUROC = AUC,
           AUPRC = prAUC)  %>%
  mutate(
...
feat_dat <- read_csv('severe-CDI/results/feature-importance_results_aggregated.csv')
sensspec_dat <- read_csv('severe-CDI/results/sensspec_results_aggregated.csv')
```
...
::::{.column width="27%" .fragment .small}
:::{.frame-box}
✅ _published x3_
:::
<br>
...
## Main papers (First / co-first) {.smaller .scrollable}
...
is_first_auth <- function(x) {
  stringr::str_starts(x, '\\*\\*Sovacool KL') | stringr::str_detect(x, "\\\\\\*Sovacool KL")
}

is_ug <- function(x) {
  stringr::str_starts(x, 'Goodin') | stringr::str_starts(x, 'Everson')
}

pubs <- read_yaml_as_tbl('src/pubs.yml') %>%
  mutate(is_first = is_first_auth(authors_md),
         is_ug = is_ug(authors_md),
         pretty_title = glue("{title_md} <br> {authors_md}"),
         journal_year = glue("{year} <br> {journal}")) %>%
  filter(!is_ug) %>%
  arrange(desc(year_mo))
```


```{r first}
pubs %>% filter(is_first) %>%
  select(pretty_title, journal_year) %>%
  add_row(pretty_title="Predicting severe _C. difficile_ infections from the taxonomic composition of the gut microbiome<br> **Sovacool KL**, Tomkovich SE, Coden ML, Rao KR, Young VB, Wiens J, Schloss PD", 
          journal_year="**2023? <br> ???**", .before = 1) %>%
  kable(col.names = c("Title, Authors", "Year, Journal")) 
```


## Other papers {.smaller .scrollable}

```{r collab}
pubs %>% filter(!is_first) %>%
  select(pretty_title, journal_year) %>%
  add_row(pretty_title="Characterizing microbiota features of _C. difficile_ infections with machine learning <br> Tomkovich SE, **Sovacool KL**, Bishop L, McBride K, Lesniak NA, Rao KR, Young VB, Schloss PD", 
          journal_year="**2023? <br> ???**", .before=1) %>%
  kable(col.names = c("Title, Authors", "Year, Journal"))
```
...
![](phd-timeline/figures/custom_no_papers-1.png)
...
```{r int}
ncases_int <- read_csv('severe-CDI/data/process/cases_int_metadata.csv') %>% nrow()
```
...
:::{.big-text}
Can we improve model performance by using both OTUs + EHRs as features?
:::

## Including EHR data {.smaller}

- Li _et al._ used 900 features extracted from the EHR
    - Manually excluded variables that are a proxy for attributable severity or encode clinician suspicion, e.g. antibiotics.

. . .

- Alternative idea (thanks Jenna!) - extract **M-CURES** score
    - Uses 9 variables from the EHR to predict clinical deterioration in COVID-19 patients.
    - Include maximum M-CURES score on day of CDI diagnosis as a feature.

. . .

> **Variables for M-CURES score:**
>
> age, respiratory rate, oxygen saturation, oxygen flow rate, pulse oximetry type
> (eg, continuous, intermittent), head-of-bed position (eg, at 30°),
> position of patient during blood pressure measurement (standing, sitting, lying),
> venous blood gas pH, and partial pressure of carbon dioxide in arterial blood
...
"age, respiratory rate, oxygen saturation, oxygen flow rate, pulse oximetry type (eg, continuous, intermittent), head-of-bed position (eg, at 30°), position of patient during blood pressure measurement (standing, sitting, lying), venous blood gas pH, and partial pressure of carbon dioxide in arterial blood"

We used FIDDLE (Flexible Data Driven Pipeline), an open source preprocessing pipeline for structured electronic health record data, to map the nine data elements to 88 binary features (each with a value of 0 or 1) describing every four hour window.
:::
...
- EHRs
    - Submit request to data office to pull 9 EHR variables from the day of diagnosis for these CDIs
    - Calculate max M-CURES scores on the day of diagnosis
    - Build models with M-CURES alone and M-CURES + OTUs to predict CDI severity.

. . .

- Draft manuscript + finalize figures
- Draft intro & discussion for dissertation

## post-PhD plans

Next position as a **Bioinformatics Engineer/Scientist** in an academic core, national lab,
nonprofit institute, or biotech/pharma company contributing to open source tools.

. . .

<br>

Submitted 16 job apps. Landed interviews with 2 potential employers:

- Qiagen
- Frederick National Lab (NIH/NCI)

:::{.notes}
I'm a fan of:

- Open science + open source software
- Building software tools to enable biomedical research

Not a fan of:

- Grant writing
- Data science without tool development
:::
...
- Qiagen
- Frederick National Lab (NIH/NCI) - **verbal offer received!**
